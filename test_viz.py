...
from visualizer import viz_all, viz_track, viz_all_unroll, viz_keyp_action_pred
import numpy as np
...
def viz_seq(args):
    ...
    with torch.no_grad():
        for data in islice(loader, 1):
            ...
            if args.keyp_pred:
                keypoints_seq, heatmaps_seq, recon_img_seq, pred_img_seq, pred_keyp_seq = model(img_seq, action_seq)
                ...
                if args.unroll:
                    #pred_img_seq = model.keyp_to_img(pred_keyp_seq)
                    pred_img_seq, pred_keyp_seq = model.unroll(img_seq, action_seq)

                    print("Pred Recon LOSS:",
                          F.mse_loss(img_seq[:, 1:], pred_img_seq, reduction='sum')/
                          (pred_img_seq.shape[0] * pred_img_seq.shape[1]))

                    pred_img_seq_np = img_torch_to_numpy(pred_img_seq)

                pred_keyp_seq_np = pred_keyp_seq.cpu().numpy()
            elif args.keyp_inverse:
                keypoints_seq, heatmaps_seq, recon_img_seq, pred_action_seq = model(img_seq)
            else:
                keypoints_seq, heatmaps_seq, recon_img_seq = model(img_seq)

            print("LOSS:", F.mse_loss(img_seq, recon_img_seq, reduction='sum')/((img_seq.shape[0]) * img_seq.shape[1]))

            print(img_seq.shape, keypoints_seq.shape, recon_img_seq.shape)

            img_seq_np, recon_img_seq_np = img_torch_to_numpy(img_seq), img_torch_to_numpy(recon_img_seq)
            ...
            for i in islice(range(num_seq),3):
                ...
                if not args.unroll:
                    viz_track(img_seq_np[i], recon_img_seq_np[i], keypoints_seq_np[i], True, 100, save_path)
                else:
                    viz_keyp_action_pred(img_seq_np[i], recon_img_seq_np[i], keypoints_seq_np[i],
                                         pred_img_seq_np[i], pred_keyp_seq_np[i], delay=100, save_path=save_path)
