...
import utils
from dynamics import VRNNKeypoints
...
class KeypointModel(pl.LightningModule):
    ...
    def __init__(self, hparams):
        ...
        self.dynamics_net = VRNNKeypoints(cfg)
    ...
    def forward(self, img_seq):
        ...
        observed_keypoints_seq_stop = keypoints_seq.detach()

        predicted_keypoints_seq, kl_div_seq = self.dynamics_net(observed_keypoints_seq_stop)


        T = self.cfg.observed_steps
        predicted_img_seq = self.keypoints_to_images_net(
            predicted_keypoints_seq.detach(),
            img_seq[:, 0].detach(),
            predicted_keypoints_seq[:, 0].detach())

        return keypoints_seq, \
               heatmaps_seq, \
               reconstructed_img_seq, \
               predicted_img_seq, \
               predicted_keypoints_seq, kl_div_seq

    def unroll(self, img_seq, T):
        keypoints_seq, _ = self.images_to_keypoints_net(img_seq)
        pred_keypoints_seq = self.dynamics_net.unroll(keypoints_seq, T)
        pred_img_seq = self.keypoints_to_images_net(pred_keypoints_seq,
                                                    img_seq[:, 0],
                                                    pred_keypoints_seq[:, 0])
    ...
    def step(self, batch, batch_idx, is_train=True):
        ...
        predicted_img_seq, predicted_keypoints_seq, kl_div_seq = self.forward(img_seq)
        ...
        vrnn_coord_loss = F.mse_loss(keypoints_seq.detach(), predicted_keypoints_seq, reduction='sum')
        vrnn_coord_loss /= (keypoints_seq.shape[0] * keypoints_seq.shape[1])

        kl_loss = torch.mean(kl_div_seq)

        loss = reconstruction_loss + \
               (heatmap_loss * self.cfg.heatmap_regularization) + \
               (temporal_loss * self.cfg.separation_loss_scale) + \
               vrnn_coord_loss + \
               (kl_loss * self.cfg.kl_loss_scale)


        pred_recon_loss = F.mse_loss(img_seq, predicted_img_seq, reduction='sum')
        pred_recon_loss /= (predicted_img_seq.shape[0] * predicted_img_seq.shape[1])
        ...
        output = {
            ...
            pfx + 'vrnn_coord_loss': vrnn_coord_loss,
            pfx + 'kl_loss': kl_loss,
    ...
    def aggregate_metrics(self, outputs, is_train=True):
        ...
        avg_vrnn_loss = torch.stack([x[pfx + 'vrnn_coord_loss'] for x in outputs]).mean()
        avg_kl_loss = torch.stack([x[pfx + 'kl_loss'] for x in outputs]).mean()
        ...
        logs = {
            ...
            pfx+'vrnn_coord_loss': avg_vrnn_loss,
            pfx+'kl_loss': avg_kl_loss,
...
def main(args):
    ...
    data_shape = {'image': (None, num_timsteps, 3, 128, 128)}
